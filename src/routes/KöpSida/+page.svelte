<script>
    ...
    },
    {
        name:"Kaffe",
        price: 100 ,
        img: "https://pricespy-75b8.kxcdn.com/product/standard/280/593908.jpg",
        amount: 1
    }
    ,{
        name:"Köttfärs",
        price: 100 ,
        img: "https://www.icagruppen.se/contentassets/73424cae1ec44e96b724ffa29eea092a/notfars.jpg",
        amount: 1
    },
    {
        name:"Falukorv",
        price: 100 ,
        img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQph_mLJ2C1JY31dU34ua2XRmdFYGArkPrNng&s",
        amount: 1
...
<div class="ShopPage">
<button class="Cartbutton" on:click={()=>toggleCart()}><img src="https://cdn0.iconfinder.com/data/icons/minimal-set-seven/32/minimal-49-512.png" alt="ShoppingCart"></button>
<div class="Cart" style="visibility: {showCart};">
    {#each Cart as item}
        <div class="CartItem">
            <h1>{item.name}</h1>
            <p>{item.price} kr</p>
            <input style="width: 30%;" type="number" bind:value={item.amount} on:input={()=>Total = Cart.reduce((acc, item) => acc + item.price * item.amount, 0)} >
            <img style="width: 30%; border-radius-10%; overflow:hidden;" src={item.img} alt={item.name}>
            <button style="remove" on:click={()=>{Total -= item.price * item.amount; Cart = Cart.filter(i => i != item)}}>Remove</button>
            <p>-------------------------------</p>
        </div>
    {/each}
...
<div class="Shop">
    {#each Items as item }
        <div class="Item">
            ...
            <h1>{item.name}</h1>
            <p>{item.price} kr</p>
            <button class="AddItem" on:click={()=>addItem(item)} >Add to Cart </button>
        </div>
...
.Cart{
    ...
    right: 0;
    flex-direction: column;
    border: solid 4px rgb(153, 103, 153);
    background-color: rgb(215, 178, 215);
    padding: 3%;
    border-radius: 10%;
    margin: 3%;
    overflow: scroll;
...
.Shop{
    ...
    overflow: scroll;
    grid-gap: 5%;
    border: dotted 4px rgb(62, 47, 77);
    padding: 3%;
    border-radius: 10%;
    overflow-x: hidden;
...
.Item{
    ...
    flex-direction: column;
    align-items: center;
    border: solid 2px rgb(71, 90, 108);
    border-radius: 10%;
...
.AddItem{
    width: 70%;
    height: 40%;
    background-color: rgb(112, 73, 125,0.7);
    color: white;
    border: none;
    border: solid 3px rgb(90, 72, 109);
    border-radius: 5%;
    margin-top: 5%;
