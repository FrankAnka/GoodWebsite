<script>
    let y = 0
    let Items=[
        {
        name: "Ägg",
        price: 400000,
        img:"data:image/jpeg;base64,",
        amount: 1
    },
    {
        name: "Päron",
        price: 40,
        img: "data:image/jpeg;base64,",
        amount: 1
    },
    {
        name: "Apelsin",
        price: 40,
        img: "https://www.menigo.se/globalassets/inriver/resources/741718_0.jpg?h=960&mode=max&quality=75&ref=181501&w=960",
        amount: 1
    },
    {
        name:"Mjölk",
        price: 1000 ,
        img: "https://assets.axfood.se/image/upload/f_auto,t_500/07310865000194_C1L1_s11",
        amount: 1

    },
    {
        name:"Kaffe",
        price: 100 ,
        img: "https://pricespy-75b8.kxcdn.com/product/standard/280/593908.jpg",
        amount: 1
    }
    ,{
        name:"Köttfärs",
        price: 100 ,
        img: "https://www.icagruppen.se/contentassets/73424cae1ec44e96b724ffa29eea092a/notfars.jpg",
        amount: 1
    },
    {
        name:"Falukorv",
        price: 100 ,
        img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQph_mLJ2C1JY31dU34ua2XRmdFYGArkPrNng&s",
        amount: 1

    },
    {
        name:"Kycklingfilé",
        price: 100 ,
        img:"https://cdn.kronfagel.se/app/uploads/2021/10/01135413/2624_Kycklingfile_460px-1.png",
        amount: 1
    },{
        name:"Rostbiff",
        price: 100 ,
        img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQPOquiA6DR3hG_zVruhO-l9JSVU-aGDU_qw&s"
        ,amount: 1
    },{
        name:"Bacon",
        price: 100 ,
        img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQbEcaHc9hSzCw60SLIx2B3ycltG29XuzPfCw&s",
        amount: 1

    },{
        name:"Monster Energy",
        price: 20 ,
        img:"https://bulkmart.ca/cdn/shop/files/MonsterEnergy-MangoLocoPunchJuice-12x473ml_1024x1024.webp?v=1721078499",
        amount: 1
    },{
        name:"Morötter",
        price: 100 ,
        img:"https://assets.axfood.se/image/upload/f_auto,t_500/07311042001546_C1N1_s02",
        amount: 1
    },{
        name:"Västerbottens ost",
        price: 140 ,
        img:"data:image/jpeg;base64,",
        amount: 1
    },{
        name:"Skogaholmslimpa",
        price: 50 ,
        img:"data:image/jpeg;base64,",
        amount: 1

    },{
        name:"Choklad",
        price: 40 ,
        img:"https://media.delitea.se/product-images/XL/marabou-mjolkchoklad-200g-0.jpg",
        amount: 1

    },{
        name:"Bregott",
        price: 80 ,
        img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSOg4URifmUWzc7qZfOd-Ny9iAVTHDMxfMwog&s",
        amount: 1
    }

]
let showCart="hidden"
let Cart = []
let Total = 0

function addItem(item){
    Total += item.price
    if(Cart.includes(item)){
        Cart[Cart.indexOf(item)].amount++
    }else{
        Cart = [...Cart, item]
    }
    console.log(Cart)
}

function toggleCart(){
    if(showCart == "hidden"){
        showCart = "visible"
    }else{
        showCart = "hidden"
    }
}

</script>
<div class="ShopPage">
<button class="Cartbutton" on:click={()=>toggleCart()}><img src="https://cdn0.iconfinder.com/data/icons/minimal-set-seven/32/minimal-49-512.png" alt="ShoppingCart"></button>
<div class="Cart" style="visibility: {showCart};">
    {#each Cart as item}
        <div class="CartItem">
            <h1>{item.name}</h1>
            <p>{item.price} kr</p>
            <input style="width: 30%;" type="number" bind:value={item.amount} on:input={()=>Total = Cart.reduce((acc, item) => acc + item.price * item.amount, 0)} >
            <img style="width: 30%; border-radius-10px; overflow:hidden;" src={item.img} alt={item.name}>
            <button style="remove" on:click={()=>{Total -= item.price * item.amount; Cart = Cart.filter(i => i != item)}}>Remove</button>
            <p>-------------------------------</p>
        </div>
    {/each}
    <h1>Total: {Total} kr</h1>
</div>
<div class="Shop">
    {#each Items as item }
        <div class="Item">
            <img src={item.img} alt={item.name}>
            <h1>{item.name}</h1>
            <p>{item.price} kr</p>
            <button class="AddItem" on:click={()=>addItem(item)} >Add to Cart </button>
        </div>
        
    {/each}


</div>
</div>
<style>

.ShopPage{
    display: flex;
    width: 100%;
    height: 100%;
    align-items: center;
    flex-direction: column;
    background-image: url("https://i0.wp.com/www.estidia.eu/wp-content/uploads/2018/04/Savin-NY-Website-Background-Web.jpg?ssl=1");
    background-size: cover  ;
}
.Cart{
    display: flex;
    flex-direction:column;
    position: absolute;
    width: 30%;
    height: 80%;
    top: 20;
    right: 0;
    flex-direction: column;
    border: solid 4px rgb(153, 103, 153);
    background-color: rgb(215, 178, 215);
    padding: 3%;
    border-radius: 10px;
    margin: 3%;
    overflow: scroll;
    color: dimgray;
}
.Cartbutton{
    display:flex;
    position: absolute;
    width: 3%;
    height:3%;
    top: 20;
    left: 97%;
    
    
}
.Shop{
    display: grid;
    width: 85%;
    height:80%;
    margin: 5%;
    grid-template-columns: repeat(4, 20%);
    grid-template-rows: repeat(4,60%);
    overflow: scroll;
    grid-gap: 5%;
    border: dotted 4px rgb(62, 47, 77);
    padding: 3%;
    border-radius: 10px;
    overflow-x: hidden;
    justify-content: space-around;



}
.Item{
    
    display: flex;
    width:  70%;
    height: 80%;
    flex-direction: column;
    align-items: center;
    border: solid 2px rgb(71, 90, 108);
    border-radius: 10px;
    align-items: space-between;
    padding: 5px;
    color: rgb(81, 81, 81);
}
.AddItem{
    width: 70%;
    height: 40px;
    background-color: rgb(112, 73, 125,0.7);
    color: white;
    border: none;
    border: solid 3px rgb(90, 72, 109);
    border-radius: 5px;
    margin-top: 5%;

}
</style>
